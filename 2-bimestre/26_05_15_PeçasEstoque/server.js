...
app.post('/enviar-mensagem', (req, res) => {
    ...
    let peca = req.body.peca; //pegue o valor de x que está no corpo da requisição e armazene-o na variável x. O cliente deve enviar um objeto JSON com uma propriedade "x" para que isso funcione corretamente.
    console.log(`Nome recebido foi: ${peca}`);
    ...
    let pecaprocurada = peca.toUpperCase();
    let estoque = [ "AMORTECEDOR" , "FILTRO DE ÓLEO", "PNEU"];
    let resp = "";
    let cont = 0
    for (let i = 0; i < estoque.length; i++ ){
        if(pecaprocurada == estoque[i]){
            cont ++;
        }
    }
    ...
    if (cont == 1){
        ...
        resp = "Peça não encontrada no sistema"
    }
